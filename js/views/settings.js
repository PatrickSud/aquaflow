...
import {
  h, icon, row, pill, sheet, toast, kv,
  download, confirmSheet, switchBtn, segmented
} from '../ui.js';
...
import { env, isInstalled, promptInstall } from '../pwa.js';
...
export default function settings(ctx) {
  ...
  const tabsEl = h('div', { style: { marginBottom: '14px' } });
  tabsEl.appendChild(segmented(
    [{ v: 'aquario', n: 'Aquário' }, { v: 'sistema', n: 'Sistema' }],
    tab,
    (v) => { tab = v; drawBody(); }
  ));
  ...
  const drawBody = () => {
    bodyWrap.innerHTML = '';
    if (tab === 'aquario') drawAquario(bodyWrap); else drawSistema(bodyWrap);
  };

  /* ---------------- aba Aquário: dados do tanque, registros e Consultor ---------------- */
  const drawAquario = (w) => {
    w.appendChild(h('div', { class: 'sec-title', text: 'Aquário' }));
    ...
    w.appendChild(h('div', { class: 'sec-title', text: 'Registros' }));
    ...
    w.appendChild(h('div', { class: 'sec-title', text: 'Consultor' }));
  ...
  /* ---------------- aba Sistema: conta, app, backup e zona de risco ---------------- */
  const drawSistema = (w) => {
    w.appendChild(h('div', { class: 'sec-title', text: 'Conta e nuvem' }));
    ...
    w.appendChild(h('div', { class: 'sec-title', text: 'Aplicativo' }));
    ...
    w.appendChild(h('div', { class: 'sec-title', text: 'Backup dos seus dados' }));
    w.appendChild(h('div', { class: 'alert info' }, icon('shield', 'ic'), h('div', {},
    ...
    w.appendChild(h('div', { class: 'sec-title', text: 'Zona de risco' }));
    ...
    w.appendChild(h('div', { class: 'note', style: { textAlign: 'center', padding: '18px 0 6px' }, text: `AquaFlow ${APP_VERSION}` }));
  };
...
function openRules() {
  ...
  return sheet({
    ...
    big: true,
    body: (b) => {
      b.appendChild(h('div', { class: 'note', style: { marginBottom: '12px' }, text: 'Estas regras estão no código do app e no prompt padrão enviado à IA. Valem no modo offline e no modo "Consultor do aquário" com IA conectada. O "Modo livre" do Consultor é a exceção proposital: nele a IA pode responder sem seguir estas regras — é o preço de poder opinar sobre o que foge dos dados registrados aqui.' }));
