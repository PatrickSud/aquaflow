...
import { SPECIES, SPECIES_ZONA, SPECIES_CAMARAO, SPECIES_PLANTA, SPECIES_BETTA } from '../model.js';
...
export function speciesCatalogView(ctx) {
  ...
  el.appendChild(h('div', { class: 'note', style: { marginBottom: '12px' } },
    'Todas as espécies disponíveis para cadastrar fauna neste aquário: as que você adicionou (por IA ou manualmente) e a base fixa do app.'));

  el.appendChild(h('div', { class: 'sec-title', text: `Cadastradas por você (${custom.length})` }));
  if (!custom.length) {
  ...
  fixed.forEach((s) => {
    const usedBy = (aq.livestock || []).filter((x) => x.spec === s.id).reduce((sum, x) => sum + (num(x.qty) || 0), 0);
    const g = specGlyph(s);
    const thumb = h('div', { style: { width: '34px', height: '34px', borderRadius: '9px', background: g.color + '22', color: g.color, display: 'grid', placeItems: 'center', flex: '0 0 auto' } }, icon(g.iconName, 'ic ic-sm'));
    c2.appendChild(row(s.n,
      `${s.sci ? s.sci + ' · ' : ''}adulto ~${s.adult} cm · ${usedBy ? `${usedBy} na fauna atual` : 'não usada na fauna atual'}`,
      { left: thumb, onClick: () => openSpeciesInfo(s) }));
  });
  el.appendChild(c2);
